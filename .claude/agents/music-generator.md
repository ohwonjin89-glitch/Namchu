...
> **레퍼런스 배정 원칙**: `generate-prompts` API(`/api/generate-prompts`, POST)를 호출하면 선택 장르의 레퍼런스를 **라운드 단위로 재셔플하며 균등 배정**해서 반환한다 — 매 라운드 전체 풀을 새로 섞어 소진하므로 한 라운드 안에서는 절대 중복되지 않고, 요청 곡 수가 풀을 초과해 다음 라운드로 넘어갈 때만 이전 레퍼런스가 다시 배정된다. 에이전트가 직접 레퍼런스를 선택·배정하지 않고 반드시 이 API를 통해 받는다.
>
> **기본값(2026-08-01~)은 AI가 레퍼런스를 참고해서 창작**한다(`styleMode` 미지정 = `"synthesize"`) — 곡마다 API가 배정한 레퍼런스(위 라운드 배정으로 전체 풀이 균등하게 순환)를 학습 예시 삼아 Styles를 매번 새로 합성하며, 레퍼런스 원문을 그대로 베끼지 않는다. `[[suno-style-synthesis]]` 스킬이 이 기본 동작의 세부 규칙이다. 사용자/오케스트레이터가 "레퍼런스 그대로", "원문 그대로 써줘" 등으로 **명시적으로 요청한 경우에만** API 호출 시 `styleMode: "reference"`를 지정한다. 8개 장르 전체에 동일하게 적용 가능. (예전 기본값이던 "레퍼런스 원문 그대로"는 요청 곡 수가 레퍼런스 풀보다 많을 때 여러 곡의 Styles가 사실상 동일해지는 사고로 이어져 2026-08-01 기본값을 전환했다 — [[project-2026072901-styles-duplication-fix]] 참고.)
