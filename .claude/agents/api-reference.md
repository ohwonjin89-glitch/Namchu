...
베이스 URL: `http://localhost:3000` (이 서버 자체에서 npm 서버가 로컬로 떠 있다)

> **과거 `http://172.28.32.1:3000`(WSL에서 Windows 호스트 게이트웨이로 접근하던 주소)이 문서 전반에 남아있었다 — 지금 파이프라인이 도는 Linux 서버에서는 이 IP가 연결되지 않거나(연결 자체가 SYN timeout, 실제 사례: 2026061401 프로젝트) 가끔 수 분 지연 후에만 응답하는 등 불안정하다. 모든 에이전트는 `localhost:3000`을 사용한다.
...
| 코드명 | Method | Endpoint | 용도 | 상태 |
|--------|--------|----------|------|------|
| `MJ_GEN` | POST | `/api/midjourney` | Midjourney V7 이미지 생성 (주 방법) | **활성화** |
| `NB2_GEN` | POST | `/api/nano-banana` | NB2(Evolink) 이미지 생성 | HOLD |
| `NB2_POLL` | GET | `/api/nano-banana?taskId={id}` | NB2 상태 폴링 | HOLD |
...
```
⚠️ `ar`은 반드시 `"16:9"` 고정
⚠️ `srefBase64`: 레퍼런스 폴더 `{referenceDir}`(Windows: `C:\suno-api\.claude\agents\reference\` / VPS: `/home/dgm/suno-api/.claude/agents/reference/`)에서 선택

**MJ_GEN 응답:** `{"images": ["url1", "url2", "url3", "url4"]}` — 문자열 배열 4장 반환, 폴링 불필요 (내부 처리)
최대 대기: 5분 (API 내부 처리)
...
```
⚠️ `size`는 반드시 `"16:9"` (숫자 형식 사용 금지)
⚠️ `quality`는 반드시 `"2K"`

**NB2_GEN 응답:** `{"taskId": "task-unified-XXXX-YYYY"}`

**NB2_POLL 응답:** `{ "status": "done|processing|error", "imageUrl": "https://..." }`
...
| MJ_GEN | 5분 (API 내부 처리) | 폴링 불필요 |
| NB2_GEN | 5분 (10초 × 30회) | HOLD |
