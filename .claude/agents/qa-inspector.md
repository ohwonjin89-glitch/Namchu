...
- **`abrupt_ending`** — 트랙 끝부분에서 "노래가 아직 진행 중이던 도중에 갑자기 뚝 끊기는" 패턴을 감지(2026-07-30 재설계, 아래 참고). 곡 끝나기 4~6초 전 구간(기준 구간)과 마지막 1.5초 구간(끝 구간)의 평균 음량을 비교해서, 끝 구간이 기준 구간 대비 8dB 이상 조용해지지 않았고 동시에 끝 구간 자체가 -30dB보다 크면(=아직 충분히 들리는 소리가 남아있으면) 오류로 판정한다.
- **`mid_silence_gap`** — 트랙 중간(시작·끝 각 5초를 제외한 구간)에 2초 이상 이어지는 무음 구간(`ffmpeg silencedetect=noise=-40dB:d=2.0`)이 있으면 오류로 판정한다.
- **`duplicate_style`** — `music-generator/prompts_log.json`에서 같은 프로젝트 내 두 트랙 이상이 Suno Styles 텍스트가 (보컬 성별 단어 차이를 빼면) 완전히 동일하면, 트랙 번호가 더 늦은 쪽을 오류곡으로 판정한다(가장 앞선 트랙 1곡만 살아남는다). `/api/generate-prompts`가 레퍼런스 풀보다 많은 곡을 요청받을 때 같은 레퍼런스를 그대로 재사용해 인트로/아웃로/리듬/악기 구성이 사실상 같은 곡이 되는 사고를 잡기 위한 검사(2026-07-30 신설, 아래 참고). API 쪽에서 재사용 곡의 Styles를 강제로 변주하도록 이미 수정했지만, 이 검사는 그 수정이 실패하거나 우회되는 경우를 잡는 방어선이다.

> ⚠️ **2026-07-30 사고 기록 (1) — 끝부분 컷오프/중간 무음**: 프로젝트 2026072901에서 "노래 끝부분이 갑자기 뚝 끊긴다" / "노래 중간에 꽤 긴 공백이 있다"는 두 가지 버그가 다수 트랙에서 발견되어 해당 영상 전체를 폐기했다. 원인 조사 결과 (1) 기존 `abrupt_ending` 검사는 **트랙 전체 평균 음량**을 기준선으로 삼고 있어, 조용한 인트로/아웃로가 있는 트랙은 평균 자체가 낮아져 사실상 거의 모든 트랙이 오탐되는(해당 프로젝트 30곡 중 30곡 전부 플래그) 무용지물 상태였고, (2) 설령 정상 탐지됐어도 WARN 전용이라 자동 격리가 안 되고 사람이 직접 들어보지 않으면 그대로 최종 영상에 실리는 구조였다. 그래서 기준선을 "트랙 전체 평균"이 아니라 "끝나기 몇 초 전 구간"으로 바꾸고(정상적인 페이드아웃/자연스러운 마무리는 대개 3초 이상에 걸쳐 서서히 조용해지므로 국소 구간 비교가 훨씬 정확하다), **오류 판정 기준(자동 격리 + badRatio 집계)으로 승격**했다. 동시에 이전까지 전혀 없었던 곡 중간 무음 구간 검사(`mid_silence_gap`)를 신설했다.

> ⚠️ **2026-07-30 사고 기록 (2) — Styles 완전 중복**: 위 (1)을 재발방지 조치한 직후 같은 프로젝트(2026072901)를 다시 검토하다가, 사용자가 "노래가 전부 비슷하다(도입부/끝나는 부분 리듬·스타일이 동일)"고 별도로 지적해 프로젝트 전체를 재차 폐기했다. `music-generator/prompts_log.json`을 분석한 결과 15곡 중 10곡이 5쌍으로 묶여 Suno Styles(악기 구성/BPM/무드 설명)가 보컬 성별 단어만 다르고 나머지는 완전히 동일했다. 원인은 `/api/generate-prompts`(`src/app/api/generate-prompts/route.ts`)의 레퍼런스 배정 로직 — 이 장르(Groove Hip-hop & Chill Pop)의 큐레이션 레퍼런스는 10개뿐인데 한 프로젝트가 15회 호출을 요구하므로, 초과분 5곡을 매번 완전히 독립적인 무작위 추첨으로 뽑아 이미 같은 배치 안에서 쓰인 레퍼런스를 그대로 재배정했고, 기본값인 "reference" 모드에서는 재사용 시에도 Styles 텍스트를 전혀 바꾸지 않았다(Claude에게 "DO NOT change"라고 못 박아 지시했기 때문). **API 코드를 수정**해 (a) 초과분 배정을 "레퍼런스 풀을 라운드 단위로 재셔플해 소진"하는 방식으로 바꾸고(같은 라운드 안에서는 절대 중복되지 않음), (b) 라운드 2 이상에서 재사용되는 곡(`forceVary`)은 `styleMode`가 "reference"여도 Claude가 해당 레퍼런스를 참고해서 Styles를 새로 변주해 쓰도록 강제했다. 동시에 이 QA 단계에도 `duplicate_style` 검사(위)를 방어선으로 신설했다. 근본적으로는 이 장르의 레퍼런스 풀(10개)이 프로젝트당 필요 곡 수(15개)보다 적다는 구조적 문제가 남아있다 — 코드 수정으로 완전 중복은 막았지만, 레퍼런스 풀을 15개 이상으로 늘리면 재사용 자체가 줄어 다양성이 더 개선된다(`gemini_analyzer.py add-curated`로 사용자가 직접 추가).
>
> **추가 정책 변경(2026-08-01)**: 위 임시조치(reference 모드 유지 + forceVary로 재사용 곡만 변주)에서 한 걸음 더 나아가, 사용자가 "레퍼런스는 참고만 하고 클로드가 항상 창작하는 구조를 기본값으로, 등록된 레퍼런스는 모두 골고루 쓰이게"라고 지시해 **기본 `styleMode`를 `"reference"`에서 `"synthesize"`로 전환**했다. 이제 모든 곡이 (재사용 여부와 무관하게) 자신에게 라운드-로빈으로 배정된 레퍼런스를 창작의 씨앗으로 삼아 Styles를 새로 합성하고, `"reference"`(원문 그대로)는 명시적으로 요청했을 때만 쓰인다. `duplicate_style` 검사는 그대로 유지 — synthesize 모드에서도 Claude가 실수로 두 곡에 거의 동일한 문장을 쓸 가능성에 대한 방어선 역할을 한다.
...
for fname in sorted(os.listdir(SELECTED)):
    ...
    # 계속 나오고 있는 도중에 파일이 그대로 끊겼는지" 검사(2026-07-30 재설계).
    # 트랙 전체 평균을 기준선으로 쓰면 조용한 인트로/아웃로가 있는 정상 트랙까지
    # 평균이 낮아져 오탐되므로(2026072901 프로젝트에서 30곡 중 30곡 오탐 확인),
    # "끝나기 4~6초 전 구간"을 기준선으로 삼아 "마지막 1.5초 구간"과 비교한다.
    # 자연스러운 페이드아웃/마무리는 이 구간에서도 이미 상당히 조용해져 있어야
    # 정상이다 — 기준 구간 대비 8dB도 조용해지지 않았고, 동시에 마지막 구간 자체가
    # 여전히 들릴 만한 수준(-30dB 초과)이면 노래가 진행 중이던 도중 끊긴 것으로 본다.
    def _mean_volume(start, seg_dur):
        if start < 0:
            seg_dur += start
            start = 0
        if seg_dur <= 0:
        ...
        stderr = subprocess.run(
            ["ffmpeg", "-y", "-ss", str(start), "-t", str(seg_dur), "-i", path,
             "-af", "volumedetect", "-f", "null", "-"],
            capture_output=True, text=True).stderr
        m = re.search(r"mean_volume:\s*(-?\d+\.?\d*) dB", stderr)
        return float(m.group(1)) if m else None

    ref_vol_db = _mean_volume(dur - 6.0, 2.0) if dur > 6.0 else None
    tail_mean_db = _mean_volume(dur - 1.5, 1.5) if dur > 1.5 else None

    if ref_vol_db is not None and tail_mean_db is not None:
        if (ref_vol_db - tail_mean_db) < 8 and tail_mean_db > -30:
            issues.append("abrupt_ending")

    # 곡 중간(시작·끝 각 5초 제외) 2초 이상 이어지는 무음 구간 검사 — 이전에는
    # 이런 검사 자체가 없었다(2026-07-30 프로젝트 2026072901에서 처음 발견된 버그,
    # 실제 사고 사례가 2.2~2.5초 구간이었어서 그보다 낮게 잡는다).
    EDGE_MARGIN = 5.0
    SILENCE_MIN_DUR = 2.0
    silence_stderr = subprocess.run(
        ["ffmpeg", "-i", path, "-af", f"silencedetect=noise=-40dB:d={SILENCE_MIN_DUR}",
         "-f", "null", "-"],
        capture_output=True, text=True).stderr
    silence_starts = [float(x) for x in re.findall(r"silence_start:\s*(-?\d+\.?\d*)", silence_stderr)]
    silence_ends = [float(x) for x in re.findall(r"silence_end:\s*(-?\d+\.?\d*)", silence_stderr)]
    mid_gaps = []
    for i, s in enumerate(silence_starts):
        e = silence_ends[i] if i < len(silence_ends) else dur
        if s > EDGE_MARGIN and e < dur - EDGE_MARGIN:
            mid_gaps.append([round(s, 1), round(e, 1)])
    if mid_gaps:
        issues.append("mid_silence_gap")
