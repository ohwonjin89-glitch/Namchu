...
**STEP 0** — 파이프라인 시작 전 이전 작업 메모리와 이전 프로젝트 회의록을 읽는다.
...
# 가장 최근 완료 프로젝트의 회의록에서 WARN/버그 항목 추출
...
- 이전 회의록 WARN/버그 — music-generator 스폰 시 "이전 파이프라인에서 발생한 주의사항"으로 사전 전달한다.
