...
```
C:\suno-api\.claude\agents\projects\{날짜번호}\
├── researcher\
│   └── research_report.md
├── strategist\
...
```bash
MEMORY_FILE="/workspace/suno-api/.claude/agents/.pipeline_memory.json"
...
| video-producer | playlist.mp4가 존재하고 5MB 이상이어야 함 | `{projectDir}\video-producer\playlist.mp4` |
| youtube-uploader | upload_result.json에 videoId, url, privacyStatus가 있어야 함 | `{projectDir}\youtube-uploader\upload_result.json` |
...
| video-producer | `video-producer/` 내 `.mp4` 파일 존재 + 5MB 이상 |
| youtube-uploader | `youtube-uploader/upload_result.json` 존재 + `videoId` 포함 |
...
RunPod 서버를 종료합니다.
재개하려면 서버 재시작 후 다시 실행 요청을 해주세요.
```

3. RunPod 서버 종료 (`completion-watcher`의 `stop_pod()` 로직과 동일한 API 호출):

```bash
source /workspace/suno-api/.env
...
[입력 파일] C:\suno-api\.claude\agents\projects\{PROJECT_ID}\researcher\research_report.md
[출력 파일] C:\suno-api\.claude\agents\projects\{PROJECT_ID}\strategist\concept_brief.json
...
- 채널:
- projectId:
- 프로젝트 경로: C:\suno-api\.claude\agents\projects\{projectId}  (예: 2026061401)
- 최종 제목:
- YouTube URL:
- 공개 상태:
...
**메모리 파일 위치**: `/workspace/suno-api/.claude/agents/.pipeline_memory.json`
(프로젝트 폴더 밖에 위치 — 모든 프로젝트에서 공유)
...
memory_path = "/workspace/suno-api/.claude/agents/.pipeline_memory.json"
