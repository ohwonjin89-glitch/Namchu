...
**qa-inspector가 `failType: code_bug`로 판정한 경우 2회를 기다리지 않고 즉시 system-developer를 호출한다.** 코드 버그는 재시도해도 동일하게 실패하기 때문이다. system-developer가 코드를 수정·커밋·push하면 orchestrator는 `git pull`로 최신 코드를 받은 뒤 해당 단계 에이전트에게 재시도를 지시한다.
