...
> 3. **동일 타입 에이전트를 같은 프로젝트에서 중복 스폰하지 않는다.** music-generator가 이미 작업 중이거나 완료됐으면 두 번째 music-generator를 절대 스폰하지 않는다. 다른 에이전트도 동일하게 적용. **중복 스폰 시 Suno API 429 rate-limit 충돌 + race condition이 발생하여 크레딧이 낭비되고 파이프라인이 망가진다.** 재시도가 필요하면 기존 에이전트에게 SendMessage로 재지시한다.
...
**STEP 0** — 파이프라인 시작 전 이전 작업 메모리, 이전 회의록, 사용자 피드백을 읽고 브리핑을 준비한다.
...
# 2) 이전 프로젝트 회의록 WARN/버그 항목
LATEST_PROJECT=$(ls -d "$REPO_DIR/.claude/agents/projects"/[0-9]* 2>/dev/null | sort | tail -1)
if [ -n "$LATEST_PROJECT" ] && [ -f "$LATEST_PROJECT/meeting_log.md" ]; then
  echo "=== 이전 프로젝트 회의록 주의사항 ($(basename $LATEST_PROJECT)) ==="
  grep -E "(WARN|버그|오류|후속 조치|⚠️|중복|rate.limit|streaming 상태|길이 이상)" "$LATEST_PROJECT/meeting_log.md" | head -30
fi

# 3) 사용자 피드백 전체 읽기
FEEDBACK_FILE="$REPO_DIR/.claude/agents/user-feedback.json"
[ -f "$FEEDBACK_FILE" ] && echo "=== 사용자 피드백 ===" && cat "$FEEDBACK_FILE"
```

읽은 내용에서 에이전트별 관련 항목을 추출해 **STEP 2의 브리핑 메시지에 포함**한다:

| 피드백 영역 | 대상 에이전트 |
|---|---|
| `topic` (주제선정 규칙) | strategist |
| `global`, `genres` (음악 생성 규칙) | music-generator |
| `image` (이미지 생성 규칙) | image-generator |

각 에이전트 브리핑 형식 (STEP 2에서 메시지에 포함):
```
[이전 파이프라인 피드백 — 반드시 반영]
{user-feedback.json에서 해당 에이전트와 관련된 항목 요약}
```
...
**STEP 2** — 팀 생성 완료 즉시 researcher, strategist, music-generator, image-generator 4개 에이전트에게 SendMessage 전송:
...
[이전 파이프라인 피드백 — 주제 선정 시 반드시 반영]
{user-feedback.json의 topic 섹션 내용 요약}
{이전 회의록에서 strategist 관련 WARN 항목}

researcher의 리포트를 수신하면 위 피드백을 반영하여 컨셉 브리프를 작성해줘.
```

```
[orchestrator → music-generator]
[이전 파이프라인 피드백 — 음악 생성 시 반드시 반영]
{user-feedback.json의 global + genres 섹션 내용 요약}
{이전 회의록에서 music-generator 관련 WARN/버그 항목}

strategist의 [회의 완료] 메시지를 수신하면 위 피드백을 반영하여 작업을 시작해줘.
```

```
[orchestrator → image-generator]
[이전 파이프라인 피드백 — 이미지 생성 시 반드시 반영]
{user-feedback.json의 image 섹션 내용 요약}
{이전 회의록에서 image-generator 관련 WARN/버그 항목}
