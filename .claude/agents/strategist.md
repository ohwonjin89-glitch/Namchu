...
트렌드 키워드가 여러 개 인기 있더라도, **서로 다른 카테고리를 하나의 주제에 혼합하지 않는다.**

카테고리 분류:
- **활동/상황**: 드라이브, 공부, 퇴근길, 새벽, 혼자 있을 때
- **장르/스타일**: 시티팝, R&B, 재즈, 어쿠스틱, 로파이
- **감정/분위기**: 감성적, 청량, 몽환, 설레는

> **잘못된 예 ❌**: "드라이브할 때 듣는 시티팝" → 활동(드라이브) + 장르(시티팝) 혼합, 두 키워드 중 어느 쪽을 검색하는 사람도 잘 걸리지 않음
>
> **올바른 예 ✅**:
> - 활동 중심: "드라이브할 때 듣기 좋은 노래" + 감성 수식어
> - 장르 중심: "새벽에 혼자 듣는 시티팝" (시티팝 단일 카테고리 + 분위기 수식어)

**규칙**: 핵심 카테고리 1개만 선택하고, 나머지는 감성 수식어나 상황 수식어로만 활용한다.
