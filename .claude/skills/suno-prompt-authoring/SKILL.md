...
- **배치 방식**: 풀 안의 레퍼런스를 라운드 단위로 재셔플해서 소진한다 — 매 라운드 전체 풀을 무작위로 섞은 뒤 앞에서부터 배정하므로, 한 라운드 안에서는 절대 중복되지 않는다. 요청 곡 수가 풀 크기를 초과해 다음 라운드로 넘어가면 이전 라운드에서 쓴 레퍼런스가 다시 배정될 수 있는데, 이때는 아래 예외를 따른다.
- **기본값: Styles는 레퍼런스에서 가져온 문장을 거의 그대로 사용**하고, **Lyrics는 항상 새로 쓴다** — 레퍼런스의 가사는 구조(파트 태그 배치)만 참고한다.
- **예외 — 레퍼런스 재사용 곡(2번째 라운드 이후)**: `styleMode: "reference"`(기본값)라도, 같은 배치 안에서 이미 쓰인 레퍼런스가 재배정된 곡은 Styles 문장을 원문 그대로 쓰지 않고 AI가 해당 레퍼런스를 참고해 새로 변주해서 쓴다(악기 페어링·BPM·믹스 질감 중 최소 하나는 달라지도록). API가 이런 곡에 `forceVary: true`를 표시해 반환하므로, music-generator는 별도 판단 없이 API가 준 `style`/`styleMode` 값을 그대로 쓰면 된다. 이 규칙이 없으면 같은 프로젝트 안에 Styles가 완전히 동일한 곡 쌍이 생긴다(2026-07-30 사고: 레퍼런스 10개 풀에 15곡을 요청해 5쌍이 통째로 중복 — 사용자가 "노래가 전부 비슷하다"고 지적해 프로젝트 폐기됨).
