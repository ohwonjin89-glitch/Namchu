...
- **배치 방식**: 풀 안의 레퍼런스를 라운드 단위로 재셔플해서 소진한다 — 매 라운드 전체 풀을 무작위로 섞은 뒤 앞에서부터 배정하므로, 한 라운드 안에서는 절대 중복되지 않는다(등록된 레퍼런스가 모두 골고루 쓰인다). 요청 곡 수가 풀 크기를 초과해 다음 라운드로 넘어가면 이전 라운드에서 쓴 레퍼런스가 다시 배정될 수 있다.
- **기본값(2026-08-01~): Styles는 AI가 배정된 레퍼런스를 참고해서 새로 창작**하고([[suno-style-synthesis]] 스킬 참고), **Lyrics는 항상 새로 쓴다** — 레퍼런스는 "정답(그대로 복사)"이 아니라 "학습 예시"다. 같은 배치 안에서도 곡마다 악기 페어링·BPM·믹스 질감 중 최소 하나는 달라지게 써서, 곡 구조가 전부 동일해지지 않게 한다. `/api/generate-prompts`는 `styleMode` 미지정 시 자동으로 이 모드로 동작한다.
- **예외 — 레퍼런스 원문 그대로 사용**: 사용자/오케스트레이터가 "레퍼런스 그대로 써줘", "원문 그대로" 등으로 **명시적으로 요청한 경우에만** API 호출 시 `styleMode: "reference"`를 지정한다 — 이때는 Styles를 레퍼런스 원문 그대로 쓴다. 단, 이 모드에서 요청 곡 수가 레퍼런스 풀을 초과해 재사용되는 곡(`forceVary: true`)은 원문 그대로 쓰지 않고 AI가 새로 변주한다 — 그렇지 않으면 같은 프로젝트 안에 Styles가 완전히 동일한 곡 쌍이 생긴다(2026-07-30 사고: 레퍼런스 10개 풀에 15곡을 요청해 5쌍이 통째로 중복 — 사용자가 "노래가 전부 비슷하다"고 지적해 프로젝트 폐기됨, 이 사고를 계기로 기본값 자체를 위처럼 synthesize로 전환했다).
