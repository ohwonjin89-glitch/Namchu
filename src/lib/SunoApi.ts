...
const logger = pino();
export const DEFAULT_MODEL = 'chirp-fenix'; // V5.5
  ...
  */
  ...
  private async launchBrowser(): Promise<BrowserContext> {
    ...
    for (const key in this.cookies) {
      if (key === '__session') continue; // 위에서 currentToken으로 이미 추가함
      const raw = this.cookies[key] + '';
      // Playwright/Chrome rejects cookie values containing control chars or separators
      const safe = /[\x00-\x1f\x7f ",;\\]/.test(raw)
  ...
  public async getCaptcha(): Promise<string|null> {
    ...
    // Direct hCaptcha solve via 2captcha API — no browser needed
    try {
      logger.info('CAPTCHA required. Solving via 2captcha hCaptcha API...');
      ...
      logger.info('2captcha hCaptcha solve failed: ' + e.message);
    }
    ...
    // Try multiple selectors for different Suno UI versions
    ...
    if (!textarea) {
      logger.info('Could not find song input textarea — skipping CAPTCHA trigger');
      await browser.close();
      return null;
    }
    ...
    const button = page.locator('button[aria-label="Create"], button:has-text("Create")').first();
    this.click(button);
